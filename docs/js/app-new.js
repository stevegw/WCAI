...
(function () {
  "use strict";
  console.log('[WCAI] app-new.js loaded OK');
  ...
  function configContext() {
    var org = config && config.company && config.company.org;
    if (!org) return '';
    return '<span style="display:inline-block;margin-left:10px;font-size:12px;font-weight:600;' +
      'color:var(--c-accent);background:var(--c-accent-006);border:1px solid var(--c-accent-020);' +
      'border-radius:4px;padding:2px 8px;vertical-align:middle;">' + esc(org) + '</span>';
  }
  ...
  var EXAMPLE_CONFIG = {
    company: {
      name: "Windchill Demo", org: "Demo", site: "wchill-vm",
      context_level: "organization", lock_preferences: true, use_flexible_change: true
    },
    groups: [
      { id: "eng",  name: "Engineering",    description: "Design, electrical, mechanical, systems, and requirements engineers" },
      { id: "mfg",  name: "Manufacturing",  description: "Manufacturing engineers and contract manufacturing" },
      { id: "qa",   name: "Quality",        description: "Quality engineers, safety auditors, and regulatory" },
      { id: "mgmt", name: "Management",     description: "Managers, project leads, and administrators" }
    ],
    people: [
      { id: "orgadmin",  name: "Org Administrator",   username: "orgadmin",  email: "orgadmin@wchill.com",  group: "mgmt" },
      { id: "chgadmin1", name: "Change Admin One",    username: "chgadmin1", email: "chgadmin1@wchill.com", group: "mgmt" },
      { id: "chgadmin2", name: "Change Admin Two",    username: "chgadmin2", email: "chgadmin2@wchill.com", group: "mgmt" },
      { id: "engineer1", name: "Engineer One",        username: "engineer1", email: "engineer1@wchill.com", group: "eng"  },
      { id: "engineer2", name: "Engineer Two",        username: "engineer2", email: "engineer2@wchill.com", group: "eng"  },
      { id: "qa1",       name: "Quality Assurance 1", username: "qa1",       email: "qa1@wchill.com",       group: "qa"   },
      { id: "qa2",       name: "Quality Assurance 2", username: "qa2",       email: "qa2@wchill.com",       group: "qa"   }
    ],
    roles: {
  ...
  function applyPrefix(pfx) {
    ...
    // People -- prefix id, username, email; update group reference
    (config.people || []).forEach(function (p) {
      var oldId = p.id || p.username;
      var newId = lo + '_' + oldId;
      p.id       = newId;
      p.username = newId;
      if (p.email) {
        var at = p.email.indexOf('@');
        if (at >= 0) p.email = newId + '@' + lo + '.' + p.email.slice(at + 1);
      }
      if (p.group && groupMap[p.group]) p.group = groupMap[p.group];
    });
  ...
  function applyTeamsPrefix(pfx) {
    ...
    // Org
    teamsConfig.org.name = pfx;
    if (teamsConfig.org.domain) {
      teamsConfig.org.domain = lo + '.' + teamsConfig.org.domain;
    }
  ...
  function renderCompany() {
    ...
    document.getElementById("main").innerHTML =
      '<h1 class="sec-title">Organization & Company' + configContext() + '</h1>' +
      '<p class="sec-desc">Start with your Windchill Organization — this is the technical container name used in all deployment scripts and must match Windchill exactly.</p>' +
      '<div class="field" style="margin-bottom:4px;">' +
        '<label>Windchill Organization Name *</label>' +
        '<input id="c-org" value="' + esc(c.org) + '" placeholder="e.g. AcmeOrg" oninput="WCAI.app.setCompany(\'org\',this.value)" style="max-width:360px;">' +
        '<p style="margin:4px 0 0;font-size:11px;color:var(--c-text-dim);">Must match exactly what appears in Windchill under <strong>Site &rarr; Utilities &rarr; Organization Administration</strong>. Case and spaces matter. Used in all generated scripts.</p>' +
      '</div>' +
      '<div class="row" style="margin-top:16px;">' +
        '<div class="field"><label>Company Display Name</label>' +
          '<input id="c-name" value="' + esc(c.name) + '" placeholder="e.g. Acme Engineering" oninput="WCAI.app.setCompany(\'name\',this.value)">' +
          '<p style="margin:4px 0 0;font-size:11px;color:var(--c-text-dim);">Used for labeling generated files only. Does not affect Windchill.</p></div>' +
  ...
  function renderGroups() {
    ...
    var html = '<h1 class="sec-title">Groups & Departments' + configContext() + '</h1>' +
  ...
  function renderPeople() {
    ...
    var groups = config.groups || [];
    var html = '<h1 class="sec-title">People' + configContext() + '</h1>' +
  ...
  function renderRoles() {
    ...
    var html = '<h1 class="sec-title">Role Mapping (Groups to Roles)' + configContext() + '</h1>' +
  ...
  function renderPrefs() {
    var html = '<h1 class="sec-title">Change Management Preferences' + configContext() + '</h1>' +
  ...
  function renderAssoc() {
    var html = '<h1 class="sec-title">Change Association Rules' + configContext() + '</h1>' +
  ...
  function renderValidate() {
    ...
    var html = '<h1 class="sec-title">Validate Configuration' + configContext() + '</h1>' +
  ...
  function renderGenerate() {
    ...
    var html = '<h1 class="sec-title">Generate Deployment Artifacts' + configContext() + '</h1>' +
  ...
  function renderChecklist() {
    try {
      ...
      document.getElementById("main").innerHTML =
        '<h1 class="sec-title">Post-Deployment Checklist' + configContext() + '</h1>' +
