...
def ask_choice(prompt: str, options: list[str], default: str = "") -> str:
    ...
    for i, opt in enumerate(options, 1):
        marker = C.green("●") if opt == default else C.dim("○")
...
def ask_multi_choice(prompt: str, options: list[tuple[str, str]],
    ...
    while True:
        for i, (key, label) in enumerate(options, 1):
            marker = C.green("✓") if key in selected else C.dim("○")
...
def banner():
    print(f"""
{C.green('═' * 60)}
{C.bold('  WINDCHILL CHANGE MANAGEMENT')}
{C.bold('  Configuration-as-Code Wizard')}
{C.green('═' * 60)}
  ...
  {C.dim('Press Ctrl+C at any time to abort.')}
  {C.dim('Press Enter to accept [defaults].')}
""")
...
def step_people(existing: list = None, groups: list = None) -> list:
    ...
    while True:
        ...
        print(f"    {C.green('✓')} Added: {name} ({username})")
...
def run_wizard(existing_config: Optional[dict] = None) -> dict:
    ...
    # Summary
    print(f"\n{C.green('═' * 60)}")
    print(f"{C.bold('  WIZARD COMPLETE')}")
    print(f"{C.green('═' * 60)}")
